...
interface TaskApi {
    ...
    @GET("tasks")
    fun tasks(@Header("Authorization") sessionToken: String, @Query("username") username: String?): Call<Response<List<TaskNetworkEntity>>>

    @GET("tasks/{task_id}")
    fun task(@Header("Authorization") sessionToken: String, @Path("task_id") task_id: Int): Call<Response<TaskNetworkEntity>>
